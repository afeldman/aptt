from torch import Tensor, topk, tensor, norm, stack, zeros
...
def topk_heatmap(heatmap: Tensor, k: int = 100):
    ...
    heatmap = F.max_pool2d(heatmap, kernel_size=3, stride=1, padding=1) * (heatmap == heatmap.max(dim=1, keepdim=True)[0])
...
def rescale_boxes(boxes: Tensor, input_size: tuple[int, int], output_size: tuple[int, int]) -> Tensor:
...
class CenterNetModel(Module):
    def __init__(self, backbone, in_channels_list, fpn_out_channels=256, num_classes=1, use_bbox=True, do_rescale=True):
        super().__init__()
        self.backbone = backbone
        self.neck = FPN(in_channels_list, fpn_out_channels)
        self.head = MultiScaleCenterNetHead(
            [fpn_out_channels] * len(in_channels_list),
...
class CenterNetDecoder(Module):
    def __init__(self, topk=100, score_thresh=0.1, do_rescale=True):
        super().__init__()
        self.topk = topk
    ...
    def forward(self, outputs, img_size):
        ...
        for b in range(B):
            ...
            for scale_out in outputs:
                tl_scores, tl_classes, tl_ys, tl_xs = topk_heatmap(scale_out["tl_heat"][b:b+1], self.topk)
                br_scores, _, br_ys, br_xs = topk_heatmap(scale_out["br_heat"][b:b+1], self.topk)
                ...
                for i, j in pairs:
                    ...
                    if score >= self.score_thresh:
                        all_scale_boxes.append(tensor([x1, y1, x2, y2, score, cls], device=tl_coords.device))
                ...
                if "bbox" in scale_out:
                    ...
                    for i, j in pairs:
                        ...
                        cls = tl_classes[0][i]
                        if score >= self.score_thresh:
                            all_scale_boxes.append(tensor([x1, y1, x2, y2, score, cls], device=reg.device))

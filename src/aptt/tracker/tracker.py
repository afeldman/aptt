...
class ParticleFilterGPU(TrackerBase):
    ...
    def __init__(
        ...
        initial_state = torch.tensor([cx, cy, w, h, 0.0, 0.0, 0.0, 0.0], 
    ...
    def predict(self) -> torch.Tensor:
        ...
        # Position update
        self.particles[:, 0] += self.particles[:, 4] + torch.randn(
            self.num_particles, device=self.device) * self.process_noise_pos
        self.particles[:, 1] += self.particles[:, 5] + torch.randn(
            self.num_particles, device=self.device) * self.process_noise_pos
        
        # Scale update (w, h)
        self.particles[:, 2] += self.particles[:, 6] + torch.randn(
            self.num_particles, device=self.device) * self.process_noise_scale
        self.particles[:, 3] += self.particles[:, 7] + torch.randn(
            self.num_particles, device=self.device) * self.process_noise_scale
        
        # Velocity update (random walk)
        self.particles[:, 4:8] += torch.randn(
            self.num_particles, 4, device=self.device) * self.process_noise_vel
        
        # Enforce box size constraints
        self.particles[:, 2] = torch.clamp(self.particles[:, 2], self.min_box_size, self.max_box_size)
        self.particles[:, 3] = torch.clamp(self.particles[:, 3], self.min_box_size, self.max_box_size)
    ...
    def _particles_to_boxes(self) -> torch.Tensor:
        ...
        Returns:
            Boxes [N, 4]
        """
        cx, cy, w, h = self.particles[:, 0], self.particles[:, 1], self.particles[:, 2], self.particles[:, 3]
        x1 = cx - w / 2
        y1 = cy - h / 2
        x2 = cx + w / 2
        y2 = cy + h / 2
    ...
    def _systematic_resample(self):
        ...
        positions = (torch.arange(self.num_particles, device=self.device, dtype=torch.float32) + 

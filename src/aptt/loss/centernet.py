...
from aptt.loss.bbox import AnkerloserBboxLoss

class CenterNetLoss(Module):
    def __init__(self, heatmap_weight=1.0, bbox_weight=1.0):
        super().__init__()
    ...
    def forward(self, preds: dict, targets: dict) -> Tensor:
        ...
        if "bbox" in preds:
            loss += self.bbox_weight * self.bbox_loss(preds["bbox"], targets["bbox"], targets["bbox_mask"])
        return loss

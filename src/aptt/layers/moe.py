...
class AuxiliaryLossFreeRouter(nn.Module):
    ...
    def forward(
        ...
        routing_weights, expert_indices = torch.topk(
            logits, self.n_expert_per_token, dim=-1
        )
    ...
    def _update_load_stats(self, expert_indices: Tensor) -> None:
        ...
        expert_counts = torch.zeros(
            self.n_routed_experts, device=expert_indices.device
        )
...
class DeepSeekMoE(nn.Module):
    ...
    Examples:
        >>> moe = DeepSeekMoE(
        ...     d_model=512, n_shared_experts=1,
        ...     n_routed_experts=64, n_expert_per_token=4
        ... )
        >>> x = torch.randn(2, 128, 512)
        >>> output, stats = moe(x)
        >>> print(output.shape)
    ...
    def __init__(
        ...
        self.shared_experts = nn.ModuleList(
            [
                FFNExpert(d_model, d_ffn, dropout)

...
class ContinualClassificationModule(BaseLightningModule):
    ...
    def __init__(
        ...
        self.head = ClassificationHead(in_channels=512, num_classes=num_classes)

        # Loss functions
        self.ce_loss = torch.nn.CrossEntropyLoss()
        self.lwf_loss = LearningWithoutForgettingLoss(alpha=lwf_alpha, temperature=lwf_temperature)
    ...
    def expand_head(self, new_classes: int):
        ...
        # Create new head
        new_head = ClassificationHead(in_channels=512, num_classes=self.num_classes)
...
def create_task_datasets(
    dataset_path: str = "./data/cifar100", n_tasks: int = 5, batch_size: int = 128
):
    ...
    val_dataset = CIFAR100(root=dataset_path, train=False, download=True, transform=transform)
    ...
    for task_id in range(n_tasks):
        ...
        # Filter val data
        val_indices = [i for i, (_, label) in enumerate(val_dataset) if label in task_classes]
        task_val = Subset(val_dataset, val_indices)

        # Create loaders
        train_loader = DataLoader(
            task_train, batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True
        )

        val_loader = DataLoader(
            task_val, batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True
        )

        task_loaders.append((train_loader, val_loader))

        print(
            f"Task {task_id + 1}: Classes {task_classes[0]}-{task_classes[-1]} "
...
def main():
    ...
    for task_id in range(n_tasks - 1):
        max_acc = max(task_accuracies[i][task_id] for i in range(task_id, n_tasks))

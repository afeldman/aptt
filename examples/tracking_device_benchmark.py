...
from aptt.tracker.tracker import ParticleFilterGPU, KalmanFilter, ParticleFilter
...
def benchmark_filter(filter_class, filter_name: str, device: str, num_frames: int = 100, **kwargs):
    ...
    for i, obs_box in enumerate(trajectory):
        ...
        predictions.append(pred_box.cpu().detach().numpy() if torch.is_tensor(pred_box) else pred_box)
...
def main():
    ...
    parser.add_argument('--all-devices', action='store_true',
                       help='Benchmark on all available devices')
    ...
    for r in all_results:
        print(f"{r['filter']:<25} {r['device']:<10} {r['fps']:<10.1f} "
